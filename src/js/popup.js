...
const ContainerHandler = class {
  ...
  updateCounters() {
    for (const count of qsA('count-frame')) {
      if (count.dataset.counter === 'gsfork') {
        count.innerHTML = this.forkCount;
      } else if (count.dataset.counter === 'custom') {
        count.innerHTML = this.customCount;
      }
    }
  }
...
const reqCode = async (obj = {}) => {
  ...
  const txt = await userjs.req(obj.code_url, 'GET', 'text').catch(MUJS.showError);
...
const createjs = (ujs, issleazy = false) => {
  for (const key in template) {
    if (Object.hasOwn(ujs, key)) continue;
  ...
  const fdesc = make('mu-js','mujs-list', {
...
function mainFN(data = {}) {
  try {
    if (!userjs.isEmpty(data)) {
      data = Array.isArray(data) ? data : [data];
      for (const ujs of data) {
        createjs(ujs, false);
      };
      MUJS.addForkCnt(data.length);
    };
  } catch (ex) {
    err(ex)
  }
...
ael(main, 'click', (evt) => {
  try {
    ...
    if (cmd === 'open-tab' && dataset.webpage) {
      ...
      dom.cl.toggle(qs('.ssearch'), 'hidden');
    } else if (cmd === 'show-filter') {
      dom.cl.toggle(qs('.fsearch'), 'hidden');
    } else if (cmd === 'save') {
      if (qs('.saveerror')) {
        qs('.saveerror').remove();
      }
      ...
      for (const i of cfg.engines) {
        if (qs(`mu-js.mujs-inlab > [data-name="${i.name}"]`)) {
          qs(`mu-js.mujs-inlab > [data-name="${i.name}"]`).checked = i.enabled;
        }
      }
      for (const i of qsA('mu-js.mujs-inlab > input[type="checkbox"]')) {
        if (!i.name.match(/((greasy|sleazy)fork|openuserjs|gi(thub|st))/gi)) {
          i.checked = cfg[i.name];
        }
      }
    } else if (cmd === 'settings') {
      // if (MUJS.unsaved && !qs('.saveerror')) {
...
const siteSearcher = qs('.ssearch input.searcher');
const filterList = qs('.fsearch input.searcher');
const btngreasy = qs('mujs-btn.greasy');
const btnissue = qs('mujs-btn.issue');
const btnhome = qs('mujs-btn.github');

siteSearcher.placeholder = MUJS.host;
ael(siteSearcher, 'change', (e) => {
  e.preventDefault();
  tabbody.innerHTML = '';
...
filterList.placeholder = lang('searcher');
ael(filterList, 'input', (e) => {
  e.preventDefault();
  const v = e.target.value;
  if (!userjs.isEmpty(v)) {
    const reg = new RegExp(v,'gi');
    for (const ujs of qsA('.frame')) {
    ...
    dom.cl.remove(qsA('.frame'), 'hidden');
  };
});
...
webext.runtime.sendMessage({}).then(mainFN);
