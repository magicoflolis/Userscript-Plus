// ==UserScript==
// @name         [Legacy] Magic Userscript+ : Show Site All UserJS
// @name:zh      [Legacy] Magic Userscript+ : 显示当前网站所有可用的UserJS脚本 Jaeger
// @name:zh-CN   [Legacy] Magic Userscript+ : 显示当前网站所有可用的UserJS脚本 Jaeger
// @name:zh-TW   [Legacy] Magic Userscript+ : 顯示當前網站所有可用的UserJS腳本 Jaeger
// @name:ja      [Legacy] Magic Userscript+ : 現在のサイトの利用可能なすべてのUserJSスクリプトを表示するJaeger
// @name:ru-RU   [Legacy] Magic Userscript+ : Показать пользовательские скрипты (UserJS) для сайта. Jaeger
// @name:ru      [Legacy] Magic Userscript+ : Показать пользовательские скрипты (UserJS) для сайта. Jaeger
...
var FetchUserjs = function () {
    ...
    _createClass(FetchUserjs, [{
        ...
        key: 'getAdultData',
        value: function getAdultData(host) {
            var adultData = GM_getResourceText('adult');
            adultData = JSON.parse(adultData);
            var adult = adultData[host];
        ...
        value: function render() {
            if (!this.isQuiet) {
                var count = this.getCountData(this.host),
                    adult = this.getAdultData(this.host);
                if (count || adult) {
