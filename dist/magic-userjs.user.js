...
(() => {
  ...
  jbox = make('jae-userjs','jbox', {
    id: 'jae_userscript_box',
  }),
  juserjs = make('div', 'jae-userscript', {
    innerHTML: `<iframe class="UserJSFrame" name="jaeFetchUserJSFrame" src="about:blank" allowTransparency="true"></iframe>`
  })
  primarycss = make('style', 'primary-stylesheet', {innerHTML: boxCSS,}),
  delay = ms => new Promise(resolve => setTimeout(resolve, ms)),
  iframe = {
    write: () => {
      const dom = juserjs.children[0],
      ...
      if (dom.tagName && 'iframe' == dom.tagName.toLowerCase()) {
        ...
        try {
          ...
          if(juserjs) {
            juserjs.innerHTML = `<span>[ERROR] loading { ${dom.name} }; [ ${d} ]</span>`;
            delay(2500).then(() => {
              juserjs.innerHTML = '';
            });
          };
    ...
    render() {
      if (!this.isQuiet) {
        if (this.getCountData(this.host)) {
          jbox.attachShadow({ mode: 'open' });
          jbox.shadowRoot.append(primarycss,juserjs);
          $('body').append(jbox);
          iframe.write();
          this.execFrameJs(juserjs.children[0].contentWindow);
          this.bindEvent();
        }
      }
    }
